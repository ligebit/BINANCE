...
async function iter() {
    ...
    if(curProfitPercent < config.maxLoss) {
        ...
        bot.telegraf.telegram.sendMessage(
            `${config.telegramUsername}`, `Торговля заблокирована. Разблокировка торговли: ${new Date(Date.now() + config.tradingLock)}`
        );

        internal.maxTotalWalletBalance = undefined;
        internal.tradingLastLockDate = Date.now();

        internal.state = `разблокировка торговли: ${new Date(Date.now() + config.tradingLock)}`
